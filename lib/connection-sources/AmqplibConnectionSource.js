...
module.exports = class AmqplibConnectionSource {
  ...
  constructor({ amqplib, decorator = new ConnectionDecorator(), connectionOptions = {}, socketOptions = {} }) {
    this._amqplib = amqplib;
    this._decorator = decorator;
    this._connectionOptions = { ...defaultConnectionOptions, ...connectionOptions };
  ...
  async _connect() {
    ...
    const connection = await this._amqplib.connect(this._connectionOptions, this._socketOptions);
  ...
  _getLoggableConnectionUrl() {
    const { protocol, username, hostname, port, vhost } = this._connectionOptions;
  ...
  _handleLostConnections(connection) {
    ...
    this._addListeners(connection, [
      { event: ScampEvent.LOST, listener: onLost },
  ...
  _makeLostEmitter(connection, event) {
    return () => {
      ...
      connection.emit(ScampEvent.LOST);
    };
  }
