const debug = require('debug')('scamp:connection-sources:RecoveringChannelSource');
...
module.exports = class RecoveringChannelSource {
  ...
  async getChannel() {
    ...
    return new Promise((resolve, reject) => this._getChannel(ChannelTypes.REGULAR, () => this._channelSource.getChannel(), resolve, reject));
  }

  async getConfirmChannel() {
    if (this._closed) throw new Error('The channel source is closed');
    return new Promise((resolve, reject) => this._getChannel(ChannelTypes.CONFIRM, () => this._channelSource.getConfirmChannel(), resolve, reject));
  }
